...
def load_deliveries_imp(pg_conn):
    with pg_conn.connection() as conn:
        with conn.cursor() as cur:
...
def load_fact_sales_imp(pg_conn):
    with pg_conn.connection() as conn:
        with conn.cursor() as cur:
...
def load_courier_ledger_imp(pg_conn):
    with pg_conn.connection() as conn:
        with conn.cursor() as cur:
...
def load_deliveries_api_imp(headers, pg_conn):
    ...
    with pg_conn.connection() as conn:
            with conn.cursor() as cur:
                cur.execute('delete from stg.delivery_system_deliveries');

    while (loading_done == False):    
        r = requests.get('https://d5d04q7d963eapoepsqr.apigw.yandexcloud.net/deliveries?limit=50&offset=' + str(offset), headers=headers)    
        response_list = json.loads(r.content)
...
with DAG(
    ...
    def load_deliveries_api(ds=None, **kwargs):
        load_deliveries_api_imp(headers, dwh_pg_connect)
